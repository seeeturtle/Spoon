...
def save_db(parsed, time):
    ...
    for lunch in parsed:
        ...
        cur.execute(
            "select exists(select 1 from lunches where date = %s)", (date,))
        has_data = cur.fetchone()[0]
        if has_data:
            continue
        cur.execute(
            "insert into lunches (date) values (%s) returning lunch_id", (date,))
        lunch_id = cur.fetchone()[0]
        for food in lunch[1]:
            cur.execute(
                "select exists(select 1 from foods where food_name = %s)", (food.name,))
            ...
            if food.stuffs:
                for fs in food.stuffs:
                    cur.execute(
                        "insert into foods_foodstuffs (food_id, foodstuff_id) values (%s, %s)", (food_id, fs))
            cur.execute(
                "insert into lunches_foods (lunch_id, food_id) values (%s, %s)", (lunch_id, food_id))
    conn.commit()
    cur.close()
    conn.close()
